...
export type CommentablePostRecord = Pick<
  PostRecord,
  "id" | "publicId" | "commentCount" | "deletedAt" | "removedAt"
>;

export type CommentRepository = {
  findCommentablePostByPublicId(publicId: string): Promise<CommentablePostRecord | null>;
...
export type CommentService = {
  ...
  deleteOwnComment(
    input: { commentId: string; authorId: string },
  ): Promise<Result<{ commentId: string; postId: string }, DeleteOwnCommentError>>;
  toggleCommentLike(
...
export function createCommentService(repository: CommentRepository): CommentService {
  return {
    async createComment(input) {
      ...
      const post = await repository.findCommentablePostByPublicId(input.publicId);
...
export function createInMemoryCommentService(state: {
  ...
  const repository: CommentRepository = {
    ...
    createComment(values) {
      ...
      const postIndex = state.posts.findIndex((post) => post.id === values.postId);
      if (postIndex !== -1) {
    ...
    markCommentDeleted(values) {
      ...
      if (existing.removedAt === null) {
        const postIndex = state.posts.findIndex((post) => post.id === existing.postId);
        if (postIndex !== -1) {
          const post = state.posts[postIndex]!;
          state.posts[postIndex] = {
            ...post,
