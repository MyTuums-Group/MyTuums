...
export const CONTACT_RATE_LIMIT = RATE_LIMIT_POLICIES.contactSubmit
...
export type ContactRateLimiter = RateLimiter
...
async function submitContact(
  ...
  const rateLimit = await deps.rateLimiter.consume({
    key: viewer
      ? createUserRateLimitKey(viewer.userId)
...
function hashContactIp(ipAddress: string | null | undefined): string | null {
  return hashRateLimitIdentifier(ipAddress)
}
