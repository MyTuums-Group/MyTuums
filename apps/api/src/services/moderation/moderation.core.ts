...
export function createInMemoryModerationService(
  ...
  function submitReport(input: SubmitReportInput) {
    ...
    if (URGENT_REPORT_REASONS.has(input.reason)) {
      moderationCase.priority = "urgent";
    }
    if (uniqueReporterCountWithinWindow(moderationCase.id, createdAt) >= 3) {
      moderationCase.priority = "urgent";
    }
  ...
  function resolveTarget(input: ReportableTargetInput): ResolvedTarget | null {
    switch (input.type) {
      case "post": {
        const post = state.posts.find((item) => item.publicId === input.publicId);
        if (!post) return null;
        return {
          targetType: "post",
          targetId: post.id,
  ...
  function canReporterSeeTarget(
    ...
    return !state.blocks.some(
      (block) =>
        (block.blockerId === reporterId && block.blockedId === target.ownerId) ||
  ...
  function caseTargetDetail(
    ...
    if (moderationCase.targetType === "post") {
      const target = state.posts.find((post) => post.id === moderationCase.targetId);
