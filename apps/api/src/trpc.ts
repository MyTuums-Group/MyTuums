...
const isAuthenticatedForAction = (action: AccountAction) =>
  t.middleware(async ({ ctx, next }) => {
    ...
    if (!isActionAllowedForAccount(ctx.accountLifecycle, action)) {
      throw new TRPCError({
        code: "FORBIDDEN",
...
export const protectedProcedure = t.procedure.use(
  isAuthenticatedForAction("protected_action")
)

/** Procedure that allows a specific account-status action. */
export function accountActionProcedure(action: AccountAction) {
  return t.procedure.use(isAuthenticatedForAction(action))
}
